...
The system SHALL generate a random non-extractable AES-256-GCM Local Device Key (LDK) after the first successful password unlock on a device. The LDK SHALL wrap the MEK using AES-256-GCM with a random IV. Both the LDK CryptoKey and the LDK-wrapped MEK ciphertext SHALL be stored together in the `local_wrapper` IndexedDB object store with `method: 'ldk'`.
...
The `local_wrapper` IndexedDB object store SHALL use a `method` field to distinguish unlock methods. The current implementation supports `method: 'ldk'`. The schema SHALL accommodate a future `method: 'pin'` record with KDF fields (kdfAlgorithm, kdfVersion, kdfParams, kdfSalt) alongside the ciphertext and IV, without requiring a schema migration.

#### Scenario: local_wrapper record has method field

- **WHEN** the system writes a `local_wrapper` record
- **THEN** the record SHALL include a `method` field set to `'ldk'`
