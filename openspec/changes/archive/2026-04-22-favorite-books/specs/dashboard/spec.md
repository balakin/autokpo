...
The system SHALL render a Dashboard page at `/dashboard` with live income statistics cards, a per-year bar chart, and a favorites section. All statistics SHALL be computed from live localStorage data via `useStats()` and `useBooks()`. The "latest book" quick-access card is replaced by the favorites section.
