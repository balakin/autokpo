...
The profile button SHALL display the user's profile image when available, or an initials-based avatar when the image is null.

#### Scenario: OAuth user sees their provider profile image

- **WHEN** the authenticated user has a non-null `image` in the session
