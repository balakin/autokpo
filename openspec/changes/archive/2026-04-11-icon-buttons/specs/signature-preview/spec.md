...
The system SHALL render an icon-only pencil button (`gravity-ui:pencil`) with `aria-label="Uredi"` and a tooltip "Uredi" in the signature preview card header. Pressing the button SHALL open a modal containing the SignatureForm.
