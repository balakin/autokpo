...
- [x] 1.2 Create `packages/eslint-config/package.json` with name `@autokpo/eslint-config`, type module, exports for `./base`, and all shared ESLint plugin dependencies
...
- [x] 3.2 Create `apps/website/eslint.config.ts` importing base from `@autokpo/eslint-config/base` and adding `astro.configs['flat/recommended']`
...
- [x] 5.1 Add `lint` task to `turbo.json` with `dependsOn: ["^lint"]` and `outputs: []`
- [x] 5.2 Add `lint:fix` task to `turbo.json` with `dependsOn: ["^lint:fix"]`, `outputs: []`, and `cache: false`
- [x] 5.3 Update root `package.json` `lint` script to `turbo run lint` and `lint:fix` to `turbo run lint:fix`

## 6. Pre-Commit Hook

- [x] 6.1 Add `pnpm lint:fix` to `.husky/pre-commit` before `pnpm lint-staged`
