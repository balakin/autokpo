...
When the viewport width is below the `lg` breakpoint, pressing the gear button SHALL open a HeroUI `Drawer` that slides in from the right edge. The Drawer SHALL fill the full viewport width and have a visible close button (X icon) in its header bar.
