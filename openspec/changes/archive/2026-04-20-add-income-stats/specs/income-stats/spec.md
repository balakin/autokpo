...
The system SHALL compute `last12MonthsIncome` as the sum of all entry incomes across all books where `datumPrometa >= subMonths(today, 12)` and `datumPrometa <= today` (inclusive). The window `last12MonthsWindow` SHALL be `{ start: subMonths(today, 12), end: today }`.
...
- **WHEN** an entry's `datumPrometa` equals `subMonths(today, 12)` exactly (same calendar day, 12 months prior)
...
Two formatters exist in `src/formatters.ts` for RSD values:
