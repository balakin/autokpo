...
The system SHALL render a Settings page at `/settings` with sections for Theme, Language, and Data. The Language section SHALL contain a functional locale selector. The Theme section SHALL contain a functional theme selector. The Data section SHALL display a logout action, a manual sync action, placeholders for export/import/clear actions, and a localized last successful sync status for this device. Timestamp rendering for the sync status SHALL rely on platform internationalization APIs and SHALL NOT require `date-fns`.
...
#### Scenario: Data section shows logout action

- **WHEN** the settings page is rendered for a signed-in user
- **THEN** the Data section SHALL display a logout action alongside sync and placeholder data actions

#### Scenario: Logout action clears local account state

- **WHEN** the user activates the logout action from the Data section
- **THEN** the app runs the logout-and-wipe flow and returns to the signed-out screen
