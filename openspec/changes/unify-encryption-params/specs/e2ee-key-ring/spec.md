...
### Requirement: encryptionVersion and wrappingVersion are stored as integer fields

**Reason**: The algorithm string alone identifies the expected params shape. A separate version integer is redundant and creates two fields that must always be checked together.
**Migration**: Remove `encryptionVersion` and `wrappingVersion` from all key ring and wrapper records. Update all read paths to check only the algorithm string. Existing records stored under the old schema must be migrated on first open.
